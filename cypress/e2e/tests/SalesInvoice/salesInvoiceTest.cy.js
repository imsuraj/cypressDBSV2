...
const {
  onCustomerReportPage,
} = require('../../../support/PageObjects/Reports/GeneralLedgerReports/CustomerReportPage.po');
const {
  onLedgerReportPage,
...
describe('Sales Invoice pages suite', () => {
  const discount = {
    firstProductDiscount: 5,
    secondProductDiscount: 2,
    billDiscount: 10,
    tradeDiscount: 5,
  };
  before(() => {
  ...
  it('Verify Header of Sales Invoice Page', () => {
    ...
    cy.get('@getBillTermsSettings').then((res) => {
      ...
      const sequence = firstBillTerm.sequence;
      const sign = firstBillTerm.sign;
      const subjectTo = firstBillTerm.subject_to;
      const transactionType = firstBillTerm.transaction_type;

      cy.log(sequence);
      cy.log(sign);
      cy.log(subjectTo);
      cy.log(transactionType);
    });
  });
  ...
  it('Verify user cannot create a sales Invoice without entering bill name for Cash Payment Mode', () => {
    ...
    onSalesInvoicePage.selectSKU(
      0,
      'Sunfeast 50-50 Top Buttery Bites 200 gm x 20 NPR 50 IN [96889]'
    );
    onSalesInvoicePage.checkStockOfSelectedSKU(0);
    onSalesInvoicePage.enterQuantity(0, 5);
  ...
  it('Verify JV, Sales A/C, Vat A/c, Customer Ledger and IRD Reports after creating a sales invoice using credit payment mode', () => {
    ...
    onSalesInvoicePage.selectSKU(
      1,
      'Sunfeast Gobbles Choco Chill 30 gm x 72 NPR 50 IN [9000992]'
    );
    onSalesInvoicePage.checkStockOfSelectedSKU(1);
    onSalesInvoicePage.enterQuantity(1, 4);
    onSalesInvoicePage.clickOnAddButon();
    onSalesInvoicePage.clickOnPlusButton();
    onSalesInvoicePage.selectSKU(
      2,
      'Sunfeast Good Day Butter Cookies 1 kg x 6 NPR 255 NP [99382]'
    );
    onSalesInvoicePage.checkStockOfSelectedSKU(2);
    onSalesInvoicePage.enterQuantity(2, 10);
    onSalesInvoicePage.clickOnAddButon();
    ...
    onSalesInvoicePage.clickOnSaveBtn();
    onSalesInvoicePage.verifySuccessMessage(
      'Success: Sales invoice has been created!'
    );
    ...
    cy.get('@invoiceNumber').then((invoiceNumber) => {
      ...
      onJvDetailPage.getDebitValueForLedger(
        'Bill Discount A/c',
        'billDiscAmount'
      );
      onJvDetailPage.getDebitValueForLedger(
                            ...
                            cy.get('@totalDebit').then((totalDebit) => {
                              cy.get('@totalCredit').then((totalCredit) => {
                                cy.get('@difference').then((difference) => {
                                  let expectedSalesAcAmount = parseFloat(
                                    subTotal.replace(/,/g, '')
                                  ).toString();
                                  let expectedBillDiscountAmount = parseFloat(
                                    billDiscountAmount.replace(/,/g, '')
                                  ).toString();
                                  let expectedTradeDiscountAmount = parseFloat(
                                    tradeDiscountAmount.replace(/,/g, '')
                                  ).toString();
                                  let expectedVatAmount = parseFloat(
                                    totalVatAmount.replace(/,/g, '')
                                  ).toString();
                                  let expectedTotalAmount = parseFloat(
                                    totalAmount.replace(/,/g, '')
                                  ).toString();

                                  let totalAmt = parseFloat(
                                    expectedTotalAmount.replace(/,/g, '')
                                  );
                                  let subTotalAmt = parseFloat(
                                    expectedSalesAcAmount.replace(/,/g, '')
                                  );
                                  let billDiscAmt = parseFloat(
                                    expectedBillDiscountAmount.replace(/,/g, '')
                                  );
                                  ...
                                  let vatAmt = parseFloat(
                                    expectedVatAmount.replace(/,/g, '')
                                  );

                                  // let expectedRoundAmount = totalAmt - subTotalAmt - billDiscAmt - tradeDiscAmt + vatAmt

                                  let actualSalesAcAmount = parseFloat(
                                    salesAmount.replace(/,/g, '')
                                  ).toString();
                                  let actualBillDiscountAmount = parseFloat(
                                    billDiscAmount.replace(/,/g, '')
                                  ).toString();
                                  let actualTradeDiscountAmount = parseFloat(
                                    tradeDiscAmount.replace(/,/g, '')
                                  ).toString();
                                  let actualVatAmount = parseFloat(
                                    vatAmount.replace(/,/g, '')
                                  ).toString();
                                  let actualRoundAmount = parseFloat(
                                    roundAmount.replace(/,/g, '')
                                  ).toString();
                                  ...
                                  let a = parseFloat(
                                    actualTotalAmount.replace(/,/g, '')
                                  );
                                  let b = parseFloat(
                                    actualBillDiscountAmount.replace(/,/g, '')
                                  );
                                  let c = parseFloat(
                                    actualTradeDiscountAmount.replace(/,/g, '')
                                  );
                                  let totalDeb = a + b + c;
                                  ...
                                  try {
                                    expect(actualSalesAcAmount).to.equal(
                                      expectedSalesAcAmount
                                    );
                                  } catch (error) {
                                    cy.log('Sales A/c Amount is not matching');
                                  }
                                  try {
                                    expect(actualBillDiscountAmount).to.equal(
                                      expectedBillDiscountAmount
                                    );
                                  } catch (error) {
                                    cy.log(
                                  ...
                                  try {
                                    expect(actualTradeDiscountAmount).to.equal(
                                      expectedTradeDiscountAmount
                                    );
                                  } catch (error) {
                                    cy.log(
                                  ...
                                  try {
                                    expect(actualVatAmount).to.equal(
                                      expectedVatAmount
                                    );
                                  } catch (error) {
                                  ...
                                  try {
                                    expect(actualTotalAmount).to.equal(
                                      expectedTotalAmount
                                    );
                                  } catch (error) {
                                  ...
                                  try {
                                    expect(actualTotalDebit).to.equal(
                                      expectedDebitTotal
                                    );
                                  } catch (error) {
                                  ...
                                  try {
                                    expect(actualTotalCredit).to.equal(
                                      expectedDebitTotal
                                    );
                                  } catch (error) {
                                  ...
                                  cy.log(
                                    '***********************************Verifying CustomerLedger Report Starting***********************************'
                                  );
                                  ...
                                  onCustomerReportPage.searchCustomerWithLedgerName(
                                    'Jain Kirana Pasal'
                                  );
                                  onCustomerReportPage.openCustomerReport(
                                    'Jain Kirana Pasal'
                                  );
                                  onCustomerReportPage.openReconciledDetails();
                                  onCustomerReportPage.getHeaderText(
                                    'headerText'
                                  );
                                  cy.get('@headerText').then((headerText) => {
                                    try {
                                      expect(headerText).to.eq(
                                        'Jain Kirana Pasal reconciled'
                                      );
                                    } catch (error) {
                                      cy.log('Header Text does not match');
                                    }
                                  });
                                  onCustomerReportPage.searchInvoiceInReportDetails(
                                    invoiceNumber
                                  );
                                  onCustomerReportPage.getDebitValueByInvoiceNum(
                                    invoiceNumber,
                                    'debitValue'
                                  );
                                  onCustomerReportPage.getCreditValueByInvoiceNum(
                                    invoiceNumber,
                                    'creditValue'
                                  );
                                  ...
                                  cy.get('@debitValue').then((debitValue) => {
                                    cy.get('@creditValue').then(
                                      (creditValue) => {
                                        let actualDebitValue = parseFloat(
                                          debitValue.replace(/,/g, '')
                                        ).toString();
                                        let actualCreditValue = parseFloat(
                                          creditValue.replace(/,/g, '')
                                        );

                                        try {
                                          expect(actualDebitValue).to.equal(
                                            expectedTotalAmount
                                          );
                                        } catch (error) {
                                          cy.log(
                                  ...
                                  cy.log(
                                    '***********************************Verifying CustomerLedger Report Finished***********************************'
                                  );
                                  cy.log(
                                    '***********************************Verifying Sales A/c Ledger Report Starting***********************************'
                                  );
                                  ...
                                  cy.get('@headerText').then((headerText) => {
                                    expect(headerText).to.eq(
                                      'SALES A/C reconciled'
                                    );
                                  });

                                  onLedgerReportPage.searchInvoiceInReportDetails(
                                    invoiceNumber
                                  );
                                  ...
                                  cy.get('@debitValueOfSalesAc').then(
                                    (debitValueOfSalesAc) => {
                                      cy.get('@creditValueOfSalesAc').then(
                                        (creditValueOfSalesAc) => {
                                          ...
                                          try {
                                            expect(actualCreditValue).to.equal(
                                              expectedSalesAcAmount
                                            );
                                          } catch (error) {
                                            cy.log(
                                  ...
                                  cy.log(
                                    '***********************************Verifying Sales A/c Ledger Report Finished***********************************'
                                  );
                                  ...
                                  cy.get('@debitValueOfSVatAc').then(
                                    (debitValueOfSVatAc) => {
                                      cy.get('@creditValueOfVatAc').then(
                                        (creditValueOfVatAc) => {
                                          // cy.log(debitValueOfSVatAc)
                                          // cy.log(creditValueOfVatAc)

                                          let actualDebitValue = parseFloat(
                                            debitValueOfSVatAc.replace(/,/g, '')
                                          );
                                          let actualCreditValue = parseFloat(
                                            creditValueOfVatAc.replace(/,/g, '')
                                          ).toString();
                                  ...
                                  cy.get('@debitValueOfBillDisctAc').then(
                                    (debitValueOfBillDisctAc) => {
                                      cy.get('@creditValueOfBillDiscAc').then(
                                        (creditValueOfBillDiscAc) => {
                                          ...
                                          try {
                                            ...
                                            cy.log(
                                              'Debit and Credit amount does not matched for Bill Discount'
                                            );
                                          }
                                        }
                                      );
                                  ...
                                  cy.get('@debitValueOfTradeDisctAc').then(
                                    (debitValueOfTradeDisctAc) => {
                                      cy.get('@creditValueOfTradeDiscAc').then(
                                        (creditValueOfTradeDiscAc) => {
                                          ...
                                          try {
                                            ...
                                            cy.log(
                                              'Debit and credit does not matched for Trade Discount'
                                            );
                                          }
                                        }
                                      );
                                  ...
                                  cy.log(
                                    '*********************************** Verifying Updated Sales VAT Report Starting ***********************************'
                                  );
                                  ...
                                  cy.get('@headerText').then((headerText) => {
                                    expect(headerText).to.eq(
                                      'Updated Sales VAT Report'
                                    );
                                  });

                                  onUpdateSalesVatReport.searchByInvoiceNumnber(
                                    invoiceNumber
                                  );
                                  onUpdateSalesVatReport.getTotalSales(
                                    invoiceNumber,
                                    'totalSales'
                                  );
                                  onUpdateSalesVatReport.getTaxExempted(
                                    invoiceNumber,
                                    'taxExempted'
                                  );
                                  onUpdateSalesVatReport.getTaxableSales(
                                    invoiceNumber,
                                    'taxableSales'
                                  );
                                  onUpdateSalesVatReport.getVat(
                                    invoiceNumber,
                                    'vat'
                                  );
                                        ...
                                        cy.get('@taxableSales').then(
                                          (taxableSales) => {
                                            cy.get('@vat').then((vat) => {
                                              ...
                                              let actualTotalSales = parseFloat(
                                                totalSales.replace(/,/g, '')
                                              ).toString();
                                              let actualTaxExempted =
                                                parseFloat(
                                                  taxExempted.replace(/,/g, '')
                                                ).toString();
                                              let actualTaxableSales =
                                                parseFloat(
                                                  taxableSales.replace(/,/g, '')
                                                ).toString();
                                              let actualVat = parseFloat(
                                                vat.replace(/,/g, '')
                                              ).toString();

                                              let expectedTotalSales =
                                                parseFloat(
                                  ...
                                  onSalesVatReport.searchByInvoiceNumnber(
                                    invoiceNumber
                                  );
                                  onSalesVatReport.getTotalSales(
                                    'totalSalesVatReport'
                                  );
                                  onSalesVatReport.getTaxExempted(
                                    'taxExemptedVatReport'
                                  );
                                  onSalesVatReport.getTaxableSales(
                                    'taxableSalesVatReport'
                                  );
                                  onSalesVatReport.getVat('vatVatReport');

                                  cy.get('@totalSalesVatReport').then(
                                    (totalSalesVatReport) => {
                                      cy.get('@taxExemptedVatReport').then(
                                        (taxExemptedVatReport) => {
                                          cy.get('@taxableSalesVatReport').then(
                                            (taxableSalesVatReport) => {
                                              cy.get('@vatVatReport').then(
                                                (vatVatReport) => {
                                                  ...
                                                  let actualTotalSales =
                                                    parseFloat(
                                                      totalSalesVatReport.replace(
                                                        /,/g,
                                                        ''
                                                      )
                                                    ).toString();
                                  ...
                                  cy.log(
                                    '*********************************** Verifying  Sales VAT Report Finsihed ***********************************'
                                  );
                                });
                              });
                            });
                            ...
                            cy.get('@totalDebit').then((totalDebit) => {
                              cy.get('@totalCredit').then((totalCredit) => {
                                cy.get('@difference').then((difference) => {
                                  ...
                                  cy.log(
                                    '***********************************Verifying CASH A/C Report Starting***********************************'
                                  );
                                  onDashboardPage.hoverMouseOverReports();
                                  onDashboardPage.hoverMouseOverGeneralLedgerReport();
                                  ...
                                  onCashBookPage.searchInvoiceInReportDetails(
                                    invoiceNumber
                                  );
                                  ...
                                  cy.log(
                                    '*********************************** Verifying CASH A/C Report Finished ***********************************'
                                  );
                                  cy.log(
                                    '*********************************** Verifying Sales A/c Ledger Report Starting***********************************'
                                  );
