...
import PurchaseInvoice from "../../../support/PageObjects/PurchaseInvoicePage/PurchaseInvoice.po";
import CreatePurchaseInvoice from "../../../support/PageObjects/PurchaseInvoicePage/CreatePurchaseInvoice.po";
...
describe("Create Purchae Invoice Test", () => {
  ...
  it.only("Check if purchage Invoice coppppppp is displayed", () => {
    ...
    //Add SKU
    createPurchaseInvoice.addNewSKU(
      "Horlicks Lite Jar 450 Gm X 24 INR 355 [UNL00201]",
      "1110BA5B",
      10,
      1,
      2,
  ...
  it.skip("Search PI", () => {
    ...
    cy.get("tbody > :nth-child(1) > :nth-child(3)")
      .should("be.visible")
      .should("have.text", vendorInvoiceNumber);
  ...
  it.skip("IRD Report -  Purchase VAT Report", () => {
    cy.visit(
      "https://qa.dbs.rosia.one/reports/ird-reports/purchase-vat-report"
    );
    cy.get(".search-input > div > .sc-jSUZER").click();
    cy.get(".sc-idXgbr").should("be.visible").click().type(vendorInvoiceNumber);
    ...
    cy.get(".MuiTableFooter-root > .MuiTableRow-root > :nth-child(4)")
      .scrollIntoView()
      .should("be.visible")
      .should("have.text", taxableAmount);
  ...
  it.skip("IRD Report -  Old Purchase VAT Report", () => {
    cy.visit(
      "https://qa.dbs.rosia.one/reports/ird-reports/old-purchase-vat-report"
    );
    cy.get(".search-input > div > .sc-jSUZER").click();
    cy.get(".sc-idXgbr").should("be.visible").click().type(vendorInvoiceNumber);
