  ...
  it.only("Verify user can create a purchase invoice by entering value for mandatory fields for cash", () => {
    ...
    cy.get("@data").then((data) => {
      data.forEach((sku, index) => {
        ...
        onCreatePI.enterDamageValue(sku.damages);
        ...
        onCreatePI.enterFirstBillTermValue(sku.excise);
        onCreatePI.enterSecondBillTermValue(sku.lineDisc);
  ...
  it("Verify JV, Ledger Report and IRD report after creating a PI for Cash", () => {
    ...
    cy.get("@data").then((data) => {
      ...
      data.forEach((sku, index) => {
        ...
        lineDiscAmt +=
          ((sku.sellable + sku.damages + sku.shortages) *
            sku.rate *
            sku.lineDisc) /
          100;
        exciseAmount +=
                                      ...
                                      (totalCredit) => {
                                        cy.get("@difference").then(
                                          (difference) => {
                                            ...
                                            try {
                                              expect(
                                                actualPurchaseAcAmount
                                              ).to.equal(
                                                expectedPurchaseAcAmount
                                              );
                                            } catch (error) {
                                              cy.log(
                                            ...
                                            onUpdatePurchaseVatReport.searchByInvoiceNumnber(
                                              purchaseDocNum
                                            );
                                            ...
                                            onPurchaseVatReport.searchByInvoiceNumnber(
                                              purchaseDocNum
                                            );
